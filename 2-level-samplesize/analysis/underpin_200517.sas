...
* remove: period_min and period_max in the infer macro's;
...
dseff=, random_statement=%str(random intercept /subject=&cluster; ),
...
data _solutions; 
	...
	* get the name of the level of the interaction variable from the format;
	...
	run;
		...
		data _solutions;set _solutions; estimate=.;stderr=.;probt=.;lower=.;upper=.;run;
		%END;
...
proc mixed data=&ds;
class &cluster &period &covars_cat ;
model percent_&outcome= &trt &period &covars &interaction_term/ solution cl outpred=_respred ; 
&random_statement;;
&repeated_statement;;
&estimate_statement;;
&contrast_statement;;
...
proc means data=_respred noprint; class &cluster &period; id &trt;
output out=_pred1 mean(pred percent_&outcome)=mean_pred mean_obs;run;
data _pred1; set _pred1; mean_resid=mean_obs-mean_pred;run;
title6 "observed vs predicted plot of clusters: by cluster";
proc sgpanel data=_pred1; panelby &cluster /onepanel;
series x=&period y=mean_pred;
...
proc contents data=dir.underpin;run;




* get dataset;
data ds0; set dir.underpin;
...
d90=died_90; label d28='died within 90 days';
...
* final ITT dataset;
data itt; set ds0; 
* remove labeling centers of discreteness;
format center; 
/* request Mark van den Boogaard/Paul Rood: ITT analyses met transitie periode
*/
run;
...
***** DATA CHECKS ***********;
title1 "data specific checks";
title2 "values cognitieve stoornissen: 0? netzo bij andere variabelen";
proc freq data=ds0; 
table cognitieve_stoornissen Alcoholmisbruik acute_opname Corticostero__den resp_insuff/ missing; run;
proc freq data=ds0; table cogn_disab alcohol urgent_admis steriods resp_insuf wrong_inclusion/ missing;run;

title2 "coding delirium type of treatment";
proc freq data=ds0; table dlr_prevtrt*dlr_prev*dlr_trt /norow nocol nopercent;run;
/*proc print data=ds0; where dlr_prevtrt=1 and dlr_trt=1;var record_id center period dlr_prev dlr_trt dlr_prevtrt;run;*/

proc print data=ds0; var record_id; where wrong_inclusion=1;run;

title2 "all data including trainingsperiods";
%rm_design(ds=ds0, cluster=center);
...
title1 "Missing Data analysis";
ods select MissPattern;
proc mi data=ds nimpute=0;
var dcf dlr_d coma_d sed_d fix_d vent_d los_icu los_hosp med_d
dlr retub readm remov fixed d28 d90;
run;

ods pdf style=statistical file="Table3_ITT_analyses.pdf" compress=9;
title1 "ITT";
...
title2 " ";
* delirium-coma free;
%sw_cont(outcome=dcf,outcome_min=0,outcome_max=28, dseff=table3, dsdescrip=table3_cont);


*delirium days;
%sw_cont(outcome=dlr_d, outcome_min=0, outcome_max=28,dseff=table3, dsdescrip=table3_cont);
	*log;
%sw_cont(outcome=dlr_d_log, outcome_min=0, outcome_max=4,dseff=table3, dsdescrip=table3_cont);

* coma days;
%sw_cont(outcome=coma_d, outcome_min=0, outcome_max=28, dseff=table3, dsdescrip=table3_cont);
	* log;
%sw_cont(outcome=coma_d_log, outcome_min=0, outcome_max=4, dseff=table3, dsdescrip=table3_cont);

* sedation days; * increase!;
%sw_cont(outcome=sed_d, outcome_min=0, outcome_max=28, dseff=table3, dsdescrip=table3_cont);
	*log;
%sw_cont(outcome=sed_d_log, outcome_min=0, outcome_max=4, dseff=table3, dsdescrip=table3_cont);
